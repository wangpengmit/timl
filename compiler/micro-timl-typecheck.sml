...
fun good_width itctx width t =
  if width = 1 then
    (case whnf ([], []) t of
       ...
       | _ => raise Impossible "array1 can only be with byte or bool"
    )
...
fun tc st_types (ctx as (ictx, tctx, ectx : econtext), st : idx) e_input =
  let
    ...
    fun main () =
      ...
      | EBinOp (opr as EBRead width', e1, e2) =>
        let
          val (e1, t1, j1, st) = tc (ctx, st) e1
          val st_e1 = st
          ...
          val () = assert_b "tc()/ERead: width'=width" $ width' = width
          val (e2, t2, j2, st) = tc (ctx, st) e2
        ...
        in
          (EBinOp (opr, e1, e2), t, j1 %%+ j2 %%+ TN (C_ERead width), st)
        end
      ...
      | EUnOp (opr as EUTiML (EUPtrProj (proj, _)), e) =>
        let
          val (e, t_e, j, st) = tc (ctx, st) e
          val t_e = whnf itctx t_e
          val t = assert_SOME $ proj_ptr proj $ assert_TPtr t_e
      ...
      | ETriOp (opr as ETWrite width', e1, e2, e3) =>
        let
          val (e1, t1, j1, st) = tc (ctx, st) e1
          val st_e1 = st
          val t1 = whnf itctx t1
          ...
          val () = assert_b "tc()/EWrite: width'=width" $ width' = width
          val (e2, t2, j2, st) = tc (ctx, st) e2
          val st_e2 = st
          val t2 = whnf itctx t2
          val i2 = case t2 of
        ...
        in
          (ETriOp (opr, e1, e2, e3), TUnit, j1 %%+ j2 %%+ j3 %%+ TN (C_EWrite width), st)
        end
      ...
      | ELet data =>
        ...
        in
          (MakeELet (e1, name, e2), t2, i1 %%+ i2, st)
        end
      (* | ELet _ => *)
      (*   let *)
