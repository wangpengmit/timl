  ...
  function trade(uint256[8] tradeValues, address[4] tradeAddresses, uint8[2] v, bytes32[4] rs) onlyAdmin returns (bool success) {
    ...
    /* tradeValues
     ...
     tradeAddressses
       ...
       [3] taker
     */
    /* PW: I suspect the author tries to avoid using local variables here because local variables are expensive (defaultly in storage, can be told to be in memory)  */
