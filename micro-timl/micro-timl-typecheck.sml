...
(* Determines when EAbsI/T must be around values. When typechecking surface MicroTiML programs, this flag should be true to ensure that the expression under EAbsI/T won't have any side effect. When typechecking MicroTiML programs generated by CPS and/or CC, this flag can be set to false because these transformations may use non-value computations to implement values. *)
val require_EAbsIT_value = ref true
    ...
    fun main () =
      ...
      | EAbsT data =>
        let
          ...
          val () = assert_b "EAbsT: is_value e" (not (!require_EAbsIT_value) orelse is_value e)
      ...
      | EAbsI data =>
        let
          ...
          val s = is_wf_sort ictx s
          val () = assert_b "EAbsI: is_value e" (not (!require_EAbsIT_value) orelse is_value e)
